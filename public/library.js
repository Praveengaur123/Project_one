...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function showBook(book) {
        ...
        if (currentTime > returnTime) {
            ...
            for(let i=0;i<returnTimeArr.length;i++ ){

                const diff=currentTimeArr[i]-returnTimeArr[i]
                sum+=Math.abs(diff)
                console.log(sum)
            }
            // console.log("difference of time",diffInMilliseconds)
            const hoursLate = Math.ceil(sum / (1000 * 60 * 60)); // Round up
    ...
    function returnBook(bookElement, fine, bookId) {
        bookElement.innerHTML = `
            <h3>Fine Amount: </h3>
            <input type=number value=${fine} id='paidFine'>
        ...
        payFineBtn.addEventListener("click", () => {
            ...
            axios.put(`http://localhost:2020/return-book/${bookId}`,{fine})
                .then(() => {
                    ...
                    document.getElementById("returnedBooks").appendChild(bookElement);
                })
